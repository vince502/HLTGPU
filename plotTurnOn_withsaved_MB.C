...
{
  const std::string recFilePath = "../store/HiForestAOD_MB326381.root";
  const std::vector<std::pair<std::string, std::string> > hltFilePath = {
      {"CPU", "./HLTTrees/HLTTree_MB_CPU.root"},
      {"CPU_Patatrack","./HLTTrees/HLTTree_MB_CPU_Patatrack.root"},
      {"GPU","./HLTTrees/HLTTree_MB_GPU.root"}
  };
  ...
  for (const auto& p : effMapIdx) {
	...
	int count =0;
	    ...
	    graph.SetMarkerStyle(count+20);
	    graph.SetLineColorAlpha(COLOR.at(c.first),1-0.3*count);
            graph.SetLineWidth((int) 3*(count));
          }
	  count++;
        }
        ...
        subdir_->cd();
        graph.Write();
	std::vector<string> tags =  {"CPU_Pat","GPU"};
	int c2 = 0;
        for (auto& g : graphM) {
          if (g.first==graphM[0].first) continue;
          g.second.Draw("samep");
        g.second.SetName(Form("%s_%s",name.c_str(),tags[c2].c_str()));
	c2++;
	std::cout << "TEST_______________________TEST c2-> " <<c2 << std::endl;
        g.second.Write();
        }
